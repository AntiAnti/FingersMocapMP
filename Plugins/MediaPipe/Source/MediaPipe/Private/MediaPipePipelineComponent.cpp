...
void UMediaPipePipelineComponent::TickComponent(float DeltaTime, ELevelTick TickType, FActorComponentTickFunction* ThisTickFunction)
{
	...
	if (Impl)
	{
		LastFrameId = (int32)Impl->GetLastFrameId();
...
int16 UMediaPipePipelineComponent::GetVideoCaptureAPI(ECV_VideoCaptureAPI Value) const
{
	UEnum* Src = StaticEnum<ECV_VideoCaptureAPI>();
	UEnum* Trg = StaticEnum<ECV_VideoCaptureAPI_ToInt>();

	FName ParamName;
	Src->GetValueAsName<ECV_VideoCaptureAPI>(Value, ParamName);
	return (int16)Trg->GetValueByName(ParamName);
}
...
bool UMediaPipePipelineComponent::Start()
{
	...
	{
		UE_LOG(LogTemp, Log, TEXT("Initialize Capture from Camera: ID=%d | API = %d | Resolution = (%d, %d) | FPS = %d"), CameraId, GetVideoCaptureAPI(CaptureAPI), (int32)CaptureResolution.X, (int32)CaptureResolution.Y, CaptureFPS);
		Impl->SetCaptureParams(CameraId, (int)GetVideoCaptureAPI(CaptureAPI), (int)CaptureResolution.X, (int)CaptureResolution.Y, CaptureFPS);
	}
	else
	{
